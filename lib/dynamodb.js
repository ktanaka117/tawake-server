...
module.exports = {
	...
	getContent: async (contentId, name) => { return await getContent(contentId, name) }
}
...
const getContent = async (contentId, name) => {
	const documentClient = new aws.DynamoDB.DocumentClient(option)
	...
	const content = await documentClient.get(params).promise()
		.catch((err) => { console.log(`DynamoDB Error!: ${err}`) })
